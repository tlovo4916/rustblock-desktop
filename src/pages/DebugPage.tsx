...
const DebugPage: React.FC = () => {
  ...
  const checkEnvironment = () => {
    const info = {
      ...
      windowTauri: typeof window !== 'undefined' && window.__TAURI__ ? 'Available' : 'Not Available',
      invokeFunction: typeof invoke !== 'undefined' ? 'Available' : 'Not Available',
      tauriApi: '@tauri-apps/api loaded',
      // 检查更多 Tauri 相关的全局变量
      windowTauriInternals: typeof (window as any).__TAURI_INTERNALS__ !== 'undefined' ? 'Available' : 'Not Available',
    };
  ...
  const testDeepSeekConnection = async () => {
    try {
      ...
      setResult(`DeepSeek Test Error: ${error}\nError Type: ${typeof error}\nError Message: ${error?.message || 'No message'}`);
    }
  };
